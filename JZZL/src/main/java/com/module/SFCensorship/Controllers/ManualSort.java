 ...
 * @describe 手动整理（用手去点一下自动整理按钮）
 * 2021年11月16日14:26:58  方法就是粘贴ArchiveAction
 */
@Controller
@RequestMapping("/ManualSort")
public class ManualSort {
    private final String operModul = "ManualSort";
    @Autowired
     ...
     */
    @RequestMapping(value = "/manualSort", method = {RequestMethod.GET,
            RequestMethod.POST})
    @ResponseBody
    @OperLog(operModul = operModul, operDesc = "手动整理", operType = OperLog.type.SELECT)
    public void manualSort(Integer caseId) {

        try {
            //查询一下是否已经开始排序了
            if (autoArchiveService.selectBaseSfcByCaseinfoid(caseId, null)) {
                return;
            }
            //根据案件id查询原始卷sfc
            FunArchiveSFCDTO thisSfc = autoArchiveService.selectOriArchiveByCaseId(caseId);
            ...
            thisSfc.setIssuspectorder(0);//未排序
...
//                    Integer BaseSeqId = oriSeq.getId();//基础卷的id
            ...
            sFCensorshipService.updateIssuspectorderBySfcId(1, thisSfc.getId());
