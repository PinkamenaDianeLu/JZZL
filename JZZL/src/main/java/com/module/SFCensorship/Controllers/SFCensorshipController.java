...
public class SFCensorshipController extends BaseFactory {
    ...
    private final  TempRemedialService tempRemedialService;
    private final
    UserSession userSession;

    @Autowired
    public SFCensorshipController(SFCensorshipService sFCensorshipService, @Qualifier("UserServiceByRedis") UserService userServiceByRedis, ArrangeArchivesService arrangeArchivesService, UserSession userSession,TempRemedialService tempRemedialService) {
        ...
        this.tempRemedialService = tempRemedialService;
    }
...
//
//            IntegralityCheckResult integralityCheckResult=tempRemedialService.integralityCheck(FunArchiveSeqMapper.INSTANCE.pcToPcDTO(needSeq));
//            if (integralityCheckResult.isOk()){
                //调用打包程序
                ...
                reValue.put("message", "success");
//            }else {
//                reValue.put("message", "checkFalse");
//                reValue.put("info", integralityCheckResult.getMessage().toString());
//            }
     ...
     * @author MrLu
     * @param caseinfoid
     * @createTime  2022/2/16 16:06
     * @return    |
      */
    @RequestMapping(value = "/selectSendErrorArchive", method = {RequestMethod.GET,
            RequestMethod.POST})
    @ResponseBody
    @OperLog(operModul = operModul, operDesc = "根据案件id查询当前所有发送失败的卷", operType = OperLog.type.SELECT)
    public String selectSendErrorArchive(String caseinfoid) {

        JSONObject reValue = new JSONObject();
        try {
            int CaseInfoId = Integer.parseInt(DecodeUrlP(caseinfoid));
            reValue.put("message", "success");
            reValue.put("value", sFCensorshipService.selectSendErrorArchive(CaseInfoId));
        } catch (Exception e) {
            e.printStackTrace();
            reValue.put("message", "error");
        }
