...
/**
 * @author MrLu
 * @createTime 2021/8/17 11:18
 * @describe 地市抽取案件
 */
@Configuration
@EnableScheduling
public class CityCaseAction implements CommandLineRunner {
    @Autowired
  ...
  //  @Scheduled(fixedRate=3000000)//每50分钟
    public void ImportCases() {
        ...
        if ("city".equals(version)) {
            //地市版
            if ("2306".equals(groupcode)) {
                //大庆
                isXz = true;
            } else if ("2327".equals(groupcode)) {
                //大兴安岭
                isXz = true;
            } else {
                System.err.println("啥地市啊？！ global.properties -> groupcode ");
                return;
            }
        } else if ("province".equals(version)||"provinceTest".equals(version)) {
            //省厅版本
            return;
        } else {
