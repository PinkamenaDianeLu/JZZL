package com.bean.zfba;

import java.util.Date;

/**
 * @author Mrlu
 * @createTime 2021/1/21
 * @describe
 */
public class WjBjz {
    private Integer id;

    private String jzfl;

    private String jqbh;

    private String wsgl;

    private String wjbm;

    private Integer wjbid;

    private String wjdm;

    private String wjzw;

    private String wjdz;

    private Integer xh;

    private Integer ym;

    private Date jlsj;

    private Date gxsj;

    private Integer scbj;

    private String sfbzj;

    private Integer jzsjbid;

    private String uuid;

    private Integer fjcbid;

    private Integer sid;

    private String sfazxt;

    private String xyrbh;

    private String xyrxm;


    public Integer getSid() {
        return sid;
    }

    public void setSid(Integer sid) {
        this.sid = sid;
    }

    public String getSfazxt() {
        return sfazxt;
    }

    public void setSfazxt(String sfazxt) {
        this.sfazxt = sfazxt;
    }

    public Integer getId() {
        return id;
    }

    public void setId(Integer id) {
        this.id = id;
    }

    public String getJzfl() {
        return jzfl;
    }

    public void setJzfl(String jzfl) {
        this.jzfl = jzfl == null ? null : jzfl.trim();
    }

    public String getJqbh() {
        return jqbh;
    }

    public void setJqbh(String jqbh) {
        this.jqbh = jqbh == null ? null : jqbh.trim();
    }

    public String getWsgl() {
        return wsgl;
    }

    public void setWsgl(String wsgl) {
        this.wsgl = wsgl == null ? null : wsgl.trim();
    }

    public String getWjbm() {
        return wjbm;
    }

    public void setWjbm(String wjbm) {
        this.wjbm = wjbm == null ? null : wjbm.trim();
    }

    public Integer getWjbid() {
        return wjbid;
    }

    public void setWjbid(Integer wjbid) {
        this.wjbid = wjbid;
    }

    public String getWjdm() {
        return wjdm;
    }

    public void setWjdm(String wjdm) {
        this.wjdm = wjdm == null ? null : wjdm.trim();
    }

    public String getWjzw() {
        return wjzw;
    }

    public void setWjzw(String wjzw) {
        this.wjzw = wjzw == null ? null : wjzw.trim();
    }

    public String getWjdz() {
        return wjdz;
    }

    public void setWjdz(String wjdz) {
        this.wjdz = wjdz == null ? null : wjdz.trim();
    }

    public Integer getXh() {
        return xh;
    }

    public void setXh(Integer xh) {
        this.xh = xh;
    }

    public Integer getYm() {
        return ym;
    }

    public void setYm(Integer ym) {
        this.ym = ym;
    }

    public Date getJlsj() {
        return jlsj;
    }

    public void setJlsj(Date jlsj) {
        this.jlsj = jlsj;
    }

    public Date getGxsj() {
        return gxsj;
    }

    public void setGxsj(Date gxsj) {
        this.gxsj = gxsj;
    }

    public Integer getScbj() {
        return scbj;
    }

    public void setScbj(Integer scbj) {
        this.scbj = scbj;
    }

    public String getSfbzj() {
        return sfbzj;
    }

    public void setSfbzj(String sfbzj) {
        this.sfbzj = sfbzj == null ? null : sfbzj.trim();
    }

    public Integer getJzsjbid() {
        return jzsjbid;
    }

    public void setJzsjbid(Integer jzsjbid) {
        this.jzsjbid = jzsjbid;
    }

    public String getUuid() {
        return uuid;
    }

    public void setUuid(String uuid) {
        this.uuid = uuid == null ? null : uuid.trim();
    }

    public Integer getFjcbid() {
        return fjcbid;
    }

    public void setFjcbid(Integer fjcbid) {
        this.fjcbid = fjcbid;
    }

    public String getXyrbh() {
        return xyrbh;
    }

    public void setXyrbh(String xyrbh) {
        this.xyrbh = xyrbh;
    }

    public String getXyrxm() {
        return xyrxm;
    }

    public void setXyrxm(String xyrxm) {
        this.xyrxm = xyrxm;
    }
}
